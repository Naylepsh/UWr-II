...
def is_same_len(word1, word2):
    if len(word1) == len(word2) or (
        len(word1) + 1 == len(word2) and word2[len(word2) - 1] in ",.;:!?") and word1[len(word1) - 1] not in ",.;:!?" or (
        len(word1) == len(word2) + 1 and word1[len(word1) - 1] in ",.;:!?" and word2[len(word2) - 1] not in ",.;:!?"):
        return True
    return False
...
def main():
    fname1 = "fraszka o zosii.txt"
    print(longest_words2(fname1))
    fname2 = "pan wolodyjowski.txt"
    print(longest_words2(fname2))
