...
(define (let*-defs? t)
  (and (list? t)
       (andmap (lambda (def) (symbol? (car def))) t)))

(define (let*-defs e)
  (cadr e))

(define (let*-defs-def defs)
  (car defs))

(define (let*-defs-tail defs)
  (cdr defs))
...
(define (cxxxr? t)
  ;;e.x. (cdddddr (list 1 2 3....))
  (define (xxxr-check-passed? x)
    (if (and (eq? (car x) 'r) (null? (cxxxr-list x)))
        #t
        (and (or (eq? (car x) 'a)
                 (eq? (car x) 'd))
             (xxxr-check-passed? (cxxxr-list x)))))
  (and (list? t)
       (= (length t) 2)
       (symbol? (car t))
       (let ([symbols (split-symbol (car t))])
         (and (>= (length symbols) 3)
              (eq? (first symbols) 'c)
              (xxxr-check-passed? (cdr symbols))))))
...
(define (eval-env e env)
  (cond [(const? e)
        ...
        [(let*? e)
         (if (= (length (let*-defs e)) 1)
             (eval-env (list 'let (let*-defs-def (let*-defs e)) (let*-expr e)) env)
             (eval-env (list 'let* (let*-defs-tail (let*-defs e)) (let*-expr e))
                       (add-to-env (let-def-var (let*-defs-def (let*-defs e)))
                                   (eval-env (let-def-expr (let*-defs-def (let*-defs e))) env)
                                   env)))]
        [(my-null? e)
         null]
        [(cons? e)
        ...
        [(cxxxr? e)
         (define (walk symbols)
           (cond
                 [(eq? (car symbols) 'a) (car (walk (cdr symbols)))]
                 [(eq? (car symbols) 'd) (cdr (walk (cdr symbols)))]
                 [(eq? (car symbols) 'c) (walk (cdr symbols))]
                 [else                   (car (cxxxr-list e))]))
         (walk (split-symbol (car e)))]
        [(pair?? e)
