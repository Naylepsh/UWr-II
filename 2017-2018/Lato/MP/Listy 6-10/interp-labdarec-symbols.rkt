  ...
  (cond [(const? e)
        ...
        [(append? e)
         ...
         (eval `((lambda-rec (append l1 l2)
                      (if (null? l1)
                          l2
                          (cons (car l1) (append (cdr l1) l2)))) ,xs ,ys))]
        [(map? e)
         (define p (second e))
         (define xs (third e))
         (eval `((lambda-rec (map p xs)
                             (if (null? xs)
                                 null
                                 (cons (p (car xs)) (map p (cdr xs))))) ,p ,xs))]
        [(cons? e)
...
(define (append? e)
  (tagged-tuple? 'my-append 3 e))

(define (map? e)
  (tagged-tuple? 'my-map 3 e))


(define (expr-reverse)
  (lambda-rec-cons '(reverse xs)
                   '(if (null? xs)
                              null
...
(eval '((lambda-rec (reverse xs)
                    (let (append (lambda-rec (append l1 l2)
                      (if (null? l1)
                          l2
                          (cons (car l1) (append (cdr l1) l2)))))
                      (let (reverse (lambda-rec (reverse xs)
                                                (if (null? xs)
                                                    null
                                                    (append (reverse (cdr xs)) (list (car xs))))))
...
(eval '(let (lcons (lambda (x f) (cons x f)))
          (let (lhead (lambda (xs) (car xs)))
            (let (ltail (lambda (xs) ((cdr xs))))
              (let (take (lambda-rec (take n xs) (cond [(= n 0) null]
                                                       [(null? xs) null]
                                                       (true (cons (lhead xs)
                                                                   (take (- n 1) (ltail xs)))))))
                (let (nats-from (lambda-rec (nats-from m) (lcons m (lambda () (nats-from (+ m 1))))))
          ...
          (let (lhead (lambda (xs) (car xs)))
            (let (ltail (lambda (xs) ((cdr xs))))
              (let (take (lambda-rec (take n xs) (cond [(= n 0) null]
                ...
                (let (nats-from (lambda-rec (nats-from m) (lcons m (lambda () (nats-from (+ m 1))))))
                  (let (nats (nats-from 0))
                    (let (test-list1 (lambda-rec (test-list1 xs n)
                                                 (if (= n 0)
                                                     (
                    (let (append (lambda-rec (append xs ys)
                                             (if (null? xs)
                                                 ys
                                                 (lcons (lhead xs)
                                                        (lambda () (append (ltail xs) ys))))))
                      (append (take 3 nats) (take 6 nats)))))))))))))))|#

(eval '((lambda x (+ (car x) (car (cdr x)))) 1 2 3))
