...
FROM users 
  ...
  JOIN posts relatedposts ON (relatedposts.id = postlinks.relatedpostid)
  WHERE linktypeid = 3 
...
ALTER TABLE badges ADD CONSTRAINT fk_user_id FOREIGN KEY (userid) REFERENCES users(id) DEFERRABLE;
