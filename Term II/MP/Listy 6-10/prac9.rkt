  ...
  (cond [(true? e) (res true st)]
        ...
        [(op? e)
         (define (op-helper op-args acc)
           ...
           ;; przenosi wynik do acc, po czym wykonuje te same procedure
           ;; dla nastepnych elementow listy.
           (if (null? op-args)
               acc
         ...
         (cond ;; Jesli brak argumentow to wywolaj po prostu ((procedura) . stan).
               ...
               ;; W przeciwnym wypadku uzyj procedury op-helper / special-op-helper,
...
(display "Primes from 3 to 200: ")
(primes-from-to 3 200)
